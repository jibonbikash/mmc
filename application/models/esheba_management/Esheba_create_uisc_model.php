...
class Esheba_create_uisc_model extends CI_Model
{
    ...
    public function get_record_list()
    {
        ...
        {
            $user['edit_link']=$CI->get_encoded_url('esheba_management/Esheba_create_uisc/index/edit/'.$user['id']);
            if($user['status']==$this->config->item('STATUS_INACTIVE'))
            {
                $user['status_text']=$CI->lang->line('PROPOSED_SERVICE');
            }
            else
            {
